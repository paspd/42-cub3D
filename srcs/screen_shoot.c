...
/*   Updated: 2021/04/03 14:19:58 by ldauga           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "../incs/cub.h"

static void create_bitmap_header(t_cub *cub, unsigned char **file, int size)
{
...
void	copy_img(t_cub *cub, unsigned char *screen)
{
	...
	while (rci_y >= 0 )
	{
		...
		{
			screen[ni_i++] = (cub->rci.addr[rci_y * cub->rci.line_length + rci_x]);
			screen[ni_i++] = (cub->rci.addr[rci_y * cub->rci.line_length + rci_x] >> 8);
			screen[ni_i++] = (cub->rci.addr[rci_y * cub->rci.line_length + rci_x] >> 16);
			screen[ni_i++] = (cub->rci.addr[rci_y * cub->rci.line_length + rci_x] >> 24);
			rci_x++;
		}
		rci_y--;
	}
